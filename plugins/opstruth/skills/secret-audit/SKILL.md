---
name: secret-audit
description: Find high-confidence secret exposure risks in static tracked files without printing matched values. Use when ChatGPT or Codex must identify possible token, key, credential, or private-key exposure by file path and count before handoff or cleanup; do not use for reading .env files, credential stores, rotation, validation, API calls, builds, tests, deployments, migrations, or mutation.
---
