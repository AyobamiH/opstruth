...
- prepared source package: `opstruth@0.2.0` pending authenticated npm publish
...
- `opstruth@0.1.3` is the latest published package. `opstruth@0.2.0` is prepared but not yet
  published.
...
The 2026-06-28 bounded production observation verified recent autonomous pg_cron run history for Wagging's intended scheduler job while preserving function-log telemetry, admin/non-admin bearer branches, and rate-limit behaviour as proof gaps. OpsTruth now has a `supabase-live` evidence-file command, plus a local telemetry-file parser, to represent these production facts without making silent production calls or committing real production evidence. The live provider adapter remains a separate hardening item until a supported, credential-safe, provider-side filtered query path can be exercised safely.
